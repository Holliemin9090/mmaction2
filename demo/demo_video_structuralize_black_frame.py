...
import os
os.environ['TORCH_HOME'] = '/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/checkpoints'
import sys
sys.path.append('/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2')
os.chdir('/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2')# the working dir has been change to allow recognition of those config files
import argparse
import copy as cp
import tempfile
import warnings
...
def visualize(args,
              ...
              max_num=5):
    h, w, _ = frames[0].shape
    frames_ = [np.zeros((h, w, 3), dtype=np.uint8) for _ in range(len(annotations) * (len(frames)//len(annotations)))]
    ...
    if pose_data_samples:
        ...
        for i, (d, f) in enumerate(zip(pose_data_samples, frames_)):
            visualizer.add_datasample(
                ...
                out_file=None,
                kpt_thr=0.3)
            # Instead of replacing the frame, copy visualized image content to the black frame
            # vis_image = visualizer.get_image()
            # np.copyto(frames_[i], vis_image)
            frames_[i] = visualizer.get_image()
            cv2.putText(frames_[i], action_result, (10, 30), FONTFACE,
                        FONTSCALE, FONTCOLOR, THICKNESS, LINETYPE)
    ...
    for i in range(na):
        ...
        for j in range(nfpa):
            ind = i * nfpa + j
            frame = frames_[ind]
            cv2.putText(frame, action_result, (10, 30), FONTFACE, FONTSCALE, FONTCOLOR, THICKNESS, LINETYPE)
...
def parse_args():
    ...
    parser.add_argument(
        '--rgb-stdet-config',
        default=(
            '/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/configs/detection/slowonly/'
    ...
    parser.add_argument(
        '--det-config',
        default='/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/demo/demo_configs/faster-rcnn_r50_fpn_2x_coco_infer.py',
    ...
    parser.add_argument(
        '--pose-config',
        default='/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/demo/demo_configs'
    ...
    parser.add_argument(
        '--skeleton-config',
        default='/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/configs/skeleton/posec3d'
    ...
    parser.add_argument(
        '--rgb-config',
        default='/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/configs/recognition/tsn/'
    ...
    parser.add_argument(
        '--video',
        default='/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/demo/test_video_structuralize.mp4',
        help='video file/url')
    parser.add_argument(
        '--label-map-stdet',
        default='/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/tools/data/ava/label_map.txt',
        help='label map file for spatio-temporal action detection')
    parser.add_argument(
        '--label-map',
        default='/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/tools/data/kinetics/label_map_k400.txt',
        help='label map file for action recognition')
    parser.add_argument(
        '--device', type=str, default='cuda:0', help='CPU/CUDA device option')
    parser.add_argument(
        '--out-filename',
        default='/datasets/work/hb-jti-fracture-project/work/openmmlab/mmaction2/my_scripts/test_stdet_recognition_output.mp4',
